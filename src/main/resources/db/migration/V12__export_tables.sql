...
CREATE TABLE `export_templates` (
    ...
    `scope_type`        TINYINT UNSIGNED NULL DEFAULT NULL COMMENT '模板默认导出范围：1=全校 2=学院 3=专业 4=班级',
    `fields_config`     JSON            NOT NULL COMMENT '字段列表模式下的列配置',
    `filter_conditions` JSON            NULL DEFAULT NULL COMMENT '默认筛选条件',
    `template_mode`     TINYINT UNSIGNED NOT NULL DEFAULT 1 COMMENT '1=字段列表模式 2=自由模板模式',
    ...
    `orientation`       TINYINT UNSIGNED NOT NULL DEFAULT 1 COMMENT '1=纵向 2=横向',
    ...
    `is_default`        TINYINT UNSIGNED NOT NULL DEFAULT 0 COMMENT '0=非默认 1=默认模板',
...
CREATE TABLE `export_template_fields` (
    ...
    `is_list`          TINYINT UNSIGNED NOT NULL DEFAULT 0 COMMENT '0=单值 1=列表循环',
...
CREATE TABLE `export_jobs` (
    ...
    `export_type`       VARCHAR(50)     NOT NULL COMMENT '导出业务类型',
    `scope_type`        TINYINT UNSIGNED NULL DEFAULT NULL COMMENT '实际导出范围',
...
CREATE TABLE `export_operation_logs` (
    ...
    `export_type`       VARCHAR(50)     NOT NULL COMMENT '导出业务类型',
    `action`            TINYINT UNSIGNED NOT NULL DEFAULT 1 COMMENT '1=创建 2=下载 3=删除',
    `scope_type`        TINYINT UNSIGNED NOT NULL COMMENT '范围类型：1=全校 2=学院 3=专业 4=班级',
    `scope_id`          BIGINT UNSIGNED NOT NULL COMMENT '对应组织表 ID',
    `filter_conditions` JSON            NULL DEFAULT NULL COMMENT '实际使用的筛选条件（快照）',
    `record_count`      INT UNSIGNED    NOT NULL DEFAULT 0 COMMENT '导出记录数',
    `is_anonymized`     TINYINT UNSIGNED NOT NULL DEFAULT 0 COMMENT '0=未脱敏 1=已脱敏',
