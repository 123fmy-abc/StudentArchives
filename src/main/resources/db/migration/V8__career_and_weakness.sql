...
CREATE TABLE `career_plans` (
    ...
    `source`           TINYINT UNSIGNED NOT NULL DEFAULT 1 COMMENT '1=手动创建 2=AI建议添加',
    `ai_suggestion_id` BIGINT UNSIGNED NULL DEFAULT NULL COMMENT '关联 improvement_suggestions.id',
    `require_confirm`  TINYINT UNSIGNED NOT NULL DEFAULT 1 COMMENT '0=直接添加 1=返回草稿待确认',
...
CREATE TABLE `career_goals` (
    ...
    `status`            TINYINT UNSIGNED NOT NULL DEFAULT 0 COMMENT '0=未开始 1=进行中 2=已完成',
...
CREATE TABLE `career_milestones` (
    ...
    `is_achieved`      TINYINT UNSIGNED NOT NULL DEFAULT 0 COMMENT '0=未完成 1=已完成',
