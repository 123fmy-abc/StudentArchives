...
CREATE TABLE `gpa_records` (
    ...
    `attempt_no`       TINYINT UNSIGNED NOT NULL DEFAULT 1 COMMENT '修读次数',
...
CREATE TABLE `grade_import_configs` (
    ...
    `has_header_row`    TINYINT UNSIGNED NOT NULL DEFAULT 1 COMMENT '0=无表头 1=首行为表头',
    `batch_size`        INT UNSIGNED    NOT NULL DEFAULT 500 COMMENT '单批次处理行数',
    `allow_overwrite`   TINYINT UNSIGNED NOT NULL DEFAULT 0 COMMENT '0=追加 1=允许覆盖',
...
CREATE TABLE `user_interests` (
    ...
    `proficiency_level` TINYINT UNSIGNED NOT NULL DEFAULT 1 COMMENT '1=入门 2=一般 3=熟练 4=精通',
    `weight`            INT UNSIGNED    NOT NULL DEFAULT 0 COMMENT '权重/出现次数',
    `sort`              INT UNSIGNED    NOT NULL DEFAULT 0 COMMENT '排序',
    `is_detail`         TINYINT UNSIGNED NOT NULL DEFAULT 0 COMMENT '0=系统标签 1=用户手动添加',
...
CREATE TABLE `score_calculations` (
    ...
    `trigger_type`     TINYINT UNSIGNED NOT NULL DEFAULT 1 COMMENT '1=手动触发 2=系统自动/定时任务',
    `operator_id`      BIGINT UNSIGNED NULL DEFAULT NULL COMMENT '操作人 ID',
    `status`           TINYINT UNSIGNED NOT NULL DEFAULT 1 COMMENT '0=计算中 1=完成 2=失败',
...
CREATE TABLE `data_completeness` (
    ...
    `semester_id`       BIGINT UNSIGNED NOT NULL COMMENT '关联 semesters.id',
    `dimension_code`    VARCHAR(50)     NOT NULL COMMENT '维度编码',
    `completeness_rate` TINYINT UNSIGNED NOT NULL DEFAULT 0 COMMENT '完整度 0-100',
...
CREATE TABLE `org_archive_summaries` (
    ...
    `org_type`                  TINYINT UNSIGNED NOT NULL COMMENT '1=学校 2=学院 3=专业 4=班级',
...
CREATE TABLE `score_recalculation_tasks` (
    `id`             BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
    `school_id`      BIGINT UNSIGNED NOT NULL COMMENT '关联 schools.id',
    `task_type`      TINYINT UNSIGNED NOT NULL COMMENT '1=指定学生 2=指定班级 3=指定学期 4=全量 5=指定专业',
    `target_id`      BIGINT UNSIGNED NULL DEFAULT NULL COMMENT '目标ID',
    `semester_id`    BIGINT UNSIGNED NULL DEFAULT NULL COMMENT '指定学期时填写',
    `status`         TINYINT UNSIGNED NOT NULL DEFAULT 0 COMMENT '0=待执行 1=执行中 2=完成 3=失败',
...
CREATE TABLE `statistics_cache` (
    ...
    `scope_type`       TINYINT UNSIGNED NULL DEFAULT NULL COMMENT '1=全校 2=学院 3=专业 4=班级',
