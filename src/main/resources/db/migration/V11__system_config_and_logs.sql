...
CREATE TABLE `scheduled_tasks` (
    ...
    `is_system`        TINYINT UNSIGNED NOT NULL DEFAULT 0 COMMENT '0=自定义 1=系统内置（不可删除）',
    `run_type`         TINYINT UNSIGNED NOT NULL DEFAULT 1 COMMENT '1=定时自动 2=手动触发',
    `max_retries`      TINYINT UNSIGNED NOT NULL DEFAULT 0 COMMENT '失败后最多重试次数',
    ...
    `last_run_status`  TINYINT UNSIGNED NULL DEFAULT NULL COMMENT '上次执行状态：0=失败 1=成功',
...
CREATE TABLE `audit_logs` (
    ...
    `action`           TINYINT UNSIGNED NOT NULL COMMENT '1=通过 2=退回 3=撤回 4=转交',
    ...
    `is_deletable`     TINYINT UNSIGNED NOT NULL DEFAULT 0 COMMENT '0=禁止普通用户删除 1=允许删除',
    `created_at`       TIMESTAMP       NOT NULL DEFAULT CURRENT_TIMESTAMP,
    `updated_at`       TIMESTAMP       NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    `deleted_at`       TIMESTAMP       NULL DEFAULT NULL COMMENT '软删除时间',
    PRIMARY KEY (`id`),
...
CREATE TABLE `system_logs` (
    ...
    `scope_type`       TINYINT UNSIGNED NULL DEFAULT NULL COMMENT '操作范围类型：1=全校 2=学院 3=专业 4=班级',
