...
CREATE TABLE `archives` (
    ...
    `duplicate_check_status` TINYINT UNSIGNED NOT NULL DEFAULT 0 COMMENT '0=未检测 1=检测中 2=疑似重复 3=已排除',
...
CREATE TABLE `duplicate_detection_rules` (
    ...
    `auto_check`          TINYINT UNSIGNED NOT NULL DEFAULT 1 COMMENT '0=手动触发 1=提交时自动检测',
    `status`              TINYINT UNSIGNED NOT NULL DEFAULT 1 COMMENT '0=禁用 1=启用',
    `created_by`          BIGINT UNSIGNED NULL DEFAULT NULL COMMENT '创建人 ID',
    `created_at`          TIMESTAMP       NOT NULL DEFAULT CURRENT_TIMESTAMP,
    `updated_at`          TIMESTAMP       NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
...
CREATE TABLE `model_versions` (
    ...
    `status`           TINYINT UNSIGNED NULL DEFAULT NULL COMMENT '该版本状态',
    `rejected_reason`  TEXT            NULL DEFAULT NULL COMMENT '退回原因',
...
CREATE TABLE `audit_comment_templates` (
    ...
    `category`         TINYINT UNSIGNED NOT NULL DEFAULT 2 COMMENT '1=通过意见 2=退回原因',
    `sort`             INT UNSIGNED    NOT NULL DEFAULT 0 COMMENT '排序',
    `status`           TINYINT UNSIGNED NOT NULL DEFAULT 1 COMMENT '0=禁用 1=启用',
...
CREATE TABLE `file_uploads` (
    ...
    `convert_status`    TINYINT UNSIGNED NOT NULL DEFAULT 0 COMMENT '0=待处理 1=已生成预览 2=失败',
    `sort_order`        INT UNSIGNED    NOT NULL DEFAULT 0 COMMENT '排序',
    `deleted_by`        BIGINT UNSIGNED NULL DEFAULT NULL COMMENT '删除者ID',
    `file_status`       TINYINT UNSIGNED NOT NULL DEFAULT 1 COMMENT '1=暂存 2=已绑定 3=已删除（软删标记）',
