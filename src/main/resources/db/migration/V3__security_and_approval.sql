...
CREATE TABLE `approval_delegations` (
    ...
    `scope_type`       INT NULL DEFAULT NULL COMMENT '委托范围类型：2=学院 3=专业 4=班级（NULL=全部）',
    ...
    `status` INT NOT NULL DEFAULT 0 COMMENT '0=待生效 1=生效中 2=已过期 3=已取消',
...
CREATE TABLE `approval_flows` (
    ...
    `is_default`          INT NOT NULL DEFAULT 0 COMMENT '0=非默认 1=默认流程',
    `status` INT NOT NULL DEFAULT 1 COMMENT '0=禁用 1=启用',
...
CREATE TABLE `approval_flow_steps` (
    ...
    `auto_assign`        INT NOT NULL DEFAULT 1 COMMENT '1=自动分配 0=手动',
    `allow_delegate`     INT NOT NULL DEFAULT 0 COMMENT '1=允许委托',
    `allow_skip`         INT NOT NULL DEFAULT 0 COMMENT '1=允许跳过',
    `allow_designate_next` INT NOT NULL DEFAULT 0 COMMENT '1=允许指定下一审批人',
    ...
    `reject_to_step`     INT NULL DEFAULT NULL COMMENT '退回后退回步骤（reject_action=return 时生效）',
...
CREATE TABLE `approval_instances` (
    ...
    `current_step`     INT NOT NULL DEFAULT 1 COMMENT '当前执行到第几步',
    `total_steps`      INT NOT NULL COMMENT '总步数',
    `status` INT NOT NULL DEFAULT 1 COMMENT '1=审批中 2=已通过 3=已退回 4=已撤回',
...
CREATE TABLE `approval_nodes` (
    ...
    `step_name`           VARCHAR(100)    NOT NULL COMMENT '步骤名称',
    `role_id`             BIGINT UNSIGNED NOT NULL COMMENT '该节点要求的角色',
    `scope_type`          INT NOT NULL COMMENT '范围类型',
    ...
    `assign_type`         INT NOT NULL DEFAULT 1 COMMENT '1=自动分配 2=手动指定 3=系统指定 4=上级指定',
    ...
    `action`              INT NULL DEFAULT NULL COMMENT '审核动作：NULL=待审核 1=通过 2=退回 3=转交 4=跳过',
...
CREATE TABLE `pending_approvals` (
    ...
    `step_no`          INT NOT NULL COMMENT '当前第几步',
    `step_name`        VARCHAR(100)    NOT NULL COMMENT '步骤名称',
    `status` INT NOT NULL DEFAULT 1 COMMENT '1=待审批 2=审批中 3=已委托',
    `created_at`       TIMESTAMP       NOT NULL DEFAULT CURRENT_TIMESTAMP,
    `updated_at`       TIMESTAMP       NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    `deleted_at`       TIMESTAMP       NULL DEFAULT NULL COMMENT '软删除时间',
    `is_deleted_null`  TINYINT(1)      GENERATED ALWAYS AS (IF(`deleted_at` IS NULL, 0, NULL)) STORED NULL,
